...
public class FireShotgun : WeaponShoot
{
    ...
    private void ProjectileShootAngle()
    {
        ...
        {
            _projectileInstance = Instantiate(_weapon.ProjectilePrefab, _shootTransforms[0].position, _weapon.TurretHead.rotation);
        ...
        else
        {
            ...
            _projectileInstance = Instantiate(_weapon.ProjectilePrefab, _shootTransforms[0].position, _weapon.TurretHead.rotation * Quaternion.Euler(_shootSpreadSeparationAngle / 2, 0, 0));
            _projectileInstance.GetComponent<Projectile>().WeaponReference = _weapon;
            _projectileInstance = Instantiate(_weapon.ProjectilePrefab, _shootTransforms[0].position, _weapon.TurretHead.rotation * Quaternion.Euler(-_shootSpreadSeparationAngle / 2, 0, 0));
            _projectileInstance.GetComponent<Projectile>().WeaponReference = _weapon;
    ...
    private void ProjectileInstantiation(int isOddOrEven)
    {
        ...
        {
            _projectileInstance = Instantiate(_weapon.ProjectilePrefab, _shootTransforms[0].position, _weapon.TurretHead.rotation * Quaternion.Euler(_shootSpreadSeparationAngle * (i / 2 + 0.5f), 0, 0));
            _projectileInstance.GetComponent<Projectile>().WeaponReference = _weapon;
            _projectileInstance = Instantiate(_weapon.ProjectilePrefab, _shootTransforms[0].position, _weapon.TurretHead.rotation * Quaternion.Euler(-_shootSpreadSeparationAngle * (i / 2 + 0.5f), 0, 0));
            _projectileInstance.GetComponent<Projectile>().WeaponReference = _weapon;
        }
